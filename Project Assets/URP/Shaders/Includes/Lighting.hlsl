...
void AddAdditionalLights_float(float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView,
    ...
    for (int i = 0; i < pixelLightCount; ++i) {
        ...
        float NdotL = (dot(WorldNormal, light.direction) + 1) / 2;
        float atten = light.distanceAttenuation * light.shadowAttenuation * GetLightIntensity(light.color);
        //half atten = light.distanceAttenuation * light.shadowAttenuation * (GetLightIntensity(light.color) + 1) / 2;
        ...
        Diffuse += thisDiffuse;
        //Diffuse *= (1 - thisDiffuse);

        Color += light.color * (thisDiffuse + thisSpecular);

        if (thisDiffuse > highestDiffuse / 2 && thisDiffuse <= highestDiffuse) {
